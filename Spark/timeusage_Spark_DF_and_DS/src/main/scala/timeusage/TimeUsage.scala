...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val fields = columnNames zip (0 to columnNames.length) map {
      case(name,0)=> StructField(name,StringType,nullable = false)
      case(name,i)=> StructField(name,DoubleType,nullable = false)
    }
    StructType(fields)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    var primaryNeedsCols= List[Column]()
    var workingActivitiesCols= List[Column]()
    var leisureCols= List[Column]()

    columnNames.foreach(name=>
      if (primaryNeeds.exists(name.startsWith)) primaryNeedsCols=col(name)::primaryNeedsCols
      else if (workingActivities.exists(name.startsWith)) workingActivitiesCols=col(name)::workingActivitiesCols
      else if (leisure.exists((name.startsWith))) leisureCols=col(name)::leisureCols
    )
    (primaryNeedsCols,workingActivitiesCols,leisureCols)
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = (primaryNeedsColumns.reduce(_ + _)/60).as("primaryNeeds") //нужно как-то уточнять типы?
    val workProjection: Column = (workColumns.reduce(_ + _)/60).as("work")
    val otherProjection: Column = (otherColumns.reduce(_ + _)/60).as("other")  //.cast("double")
    df
...
//        r.getAs("primaryNeeds"),r.getAs("work"),r.getAs("other")))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      .groupBy($"working".as[String],$"sex".as[String],$"age".as[String])
      .agg(round(avg($"primaryNeeds"),1).as[Double].as("primaryNeeds"),round(avg($"work"),1).as[Double].as("work"),round(avg($"other"),1).as[Double].as("other"))
