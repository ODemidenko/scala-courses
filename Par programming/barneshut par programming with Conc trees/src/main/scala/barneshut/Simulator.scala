...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val bds = new Boundaries
    bds.minX= boundaries.minX min body.x
    bds.maxX= boundaries.maxX max body.x
    bds.minY= boundaries.minY min body.y
    bds.maxY= boundaries.maxY max body.y
    bds
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val bds= new Boundaries
    bds.maxY=a.maxY max b.maxY
    bds.maxX=a.maxX max b.maxX
    bds.minX=a.minX min b.minX
    bds.minY=a.minY min b.minY
    bds
  }
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries,sectorPrecision = SECTOR_PRECISION))(
      _ += _,
      _ combine _
    )
  }
