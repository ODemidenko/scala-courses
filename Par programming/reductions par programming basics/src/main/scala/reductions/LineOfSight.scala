...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var i=from
    var max=input(0)
    while (i<until) {if (i!=0) max=math.max(input(i)/i,max);
      i+=1}
    max
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end-from<=threshold) Leaf(from,end,upsweepSequential(input,from,end))
      else {val middle =from+(end-from)/2
        val (l,r) = parallel(upsweep(input,from,middle,threshold),
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit =
    tree match {
      case node@Node(l,r)=>val (_,_)=parallel(downsweep(input,output,startingAngle,l),
