...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    (for {point <- points} yield (findClosest(point,means), point)
      ).groupBy(_._1) map {case (k,v)=>k->v.map(_._2)}
//      key=>map.get(key).orElse(Ge)
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    for (oldMean<-oldMeans)
      yield findAverage(oldMean,
        classified.getOrElse(oldMean,Vector[Point]()) )
//        Hint: Make sure you use the findAverage method that is predefined for you.
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (oldMeans zip newMeans) filter {case (old,newMean)=>old.squareDistance(newMean)>eta} isEmpty
  }
